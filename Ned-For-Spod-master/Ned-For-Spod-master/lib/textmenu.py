...
class Textmenu(Menu, object):
    ...
    def __init__(self, window, font, title, **kwargs):
        ...
        if kwargs.get("text_centered") is not None:
            ...
            self.centered_text = TEXT_CENTERED
        # Color de la fuente del texto
        if kwargs.get("font_text_color") is not None:
            self.font_textcolor = kwargs.get("font_text_color")
        else:
            self.font_textcolor = TEXT_FONT_COLOR
        # Tamaño de la fuente del texto
        if kwargs.get("font_text_size") is not None:
            self.font_textsize = kwargs.get("font_text_size")
        else:
            self.font_textsize = MENU_FONT_TEXT_SIZE
        # Fuente del texto
        self.fonttext = pygame.font.Font(font, self.font_textsize)
        # Distancia vertical entre linas
        if kwargs.get("text_margin") is not None:
            self.textdy = kwargs.get("text_margin")
        else:
            self.textdy = TEXT_MARGIN
        # Variables independientes de la clase
        self.text = []
        # Punto a dibujar el texto
        if kwargs.get("draw_text_region") is not None:
            self.drawTextRegionX = kwargs.get("draw_text_region")
        else:
            self.drawTextRegionX = TEXT_DRAW_X
        self.posTextX = int(
            self.width * (self.drawTextRegionX / 100.0)) + self.posx
    ...
    def add_text(self, text):
        ...
        self.actual.posOptionY += -self.actual.font_textsize / 2 - self.actual.textdy / 2
